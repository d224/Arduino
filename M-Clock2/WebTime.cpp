...
void WebTime::Start()
{
  ...
  timer0_1sec = timerBegin( 1000 ); // Set timer frequency to 1ms
  timerAttachInterrupt(timer0_1sec, &onTimer0_1sec);
  timerAlarm(timer0_1sec, 1000, true, 0); //1000 ms
...
{
  static uint32_t next_min = _SS_UP / 60 * 61 ;
  if (_SS_UP >= next_min)
  {
    next_min += 60;
