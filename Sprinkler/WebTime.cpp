...
bool     WebTime::m_geonames_updated = true;
...
{
  Serial.println("WebTimeTask started");
  ...
  for(;;)
  {
    ...
    {
      WebTime::PrintTime();
      if( (!WebTime::m_geonames_updated ) && ( geonames_retry > 0 ) )   
      {
...
{
   Serial.printf("# %02d:%02d:%02d :%d \n", WebTime::_HH, WebTime::_MM, WebTime::_SS, WebTime::m_secSinceMidNight);
   /*
  if (isDay())
    Serial.print("Day\n");
  else
