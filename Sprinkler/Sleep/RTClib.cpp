...
*/
/**************************************************************************/
//https://github.com/jarzebski/Arduino-DS3231/blob/master/
void RTC_DS3231::setAlarm1(uint8_t dydw, uint8_t hour, uint8_t minute, uint8_t second, DS3231_alarm1_t mode, bool armed)
{
    ...
    Wire.beginTransmission(DS3231_ADDRESS);
    Wire._I2C_WRITE(DS3231_REG_ALARM_1);
    Wire._I2C_WRITE(second);
    Wire._I2C_WRITE(minute);
    Wire._I2C_WRITE(hour);
    Wire._I2C_WRITE(dydw);
    Wire.endTransmission();
...
*/
/**************************************************************************/
void RTC_DS3231::armAlarm1(bool armed)
{
    uint8_t value;
    value = read_i2c_register(DS3231_ADDRESS, DS3231_CONTROL);
	...
	write_i2c_register(DS3231_ADDRESS, DS3231_CONTROL, value);
}

bool RTC_DS3231::isArmed1(void)
{
    uint8_t value;
    value = read_i2c_register(DS3231_ADDRESS, DS3231_CONTROL);
    value &= 0b00000001;
...
void RTC_DS3231::clearAlarm1(void)
{
    uint8_t value;
    value = read_i2c_register(DS3231_ADDRESS, DS3231_CONTROL);
    value &= 0b11111110;
	write_i2c_register(DS3231_ADDRESS, DS3231_STATUSREG, value);
}
bool RTC_DS3231::isAlarm1(bool clear)
{
    uint8_t alarm;
	alarm = read_i2c_register(DS3231_ADDRESS, DS3231_STATUSREG);
    alarm &= 0b00000001;
    if (alarm && clear)
    {
        clearAlarm1();
...
void RTC_DS3231::enableSqwOutput(bool enabled)
{
    uint8_t value;
...
void RTC_DS3231::enableInterruptControl(bool enabled)
{
    uint8_t value;
...
void RTC_DS3231::enableAlarm1Interrupt(bool enabled)
{
    uint8_t value;
    value = read_i2c_register(DS3231_ADDRESS, DS3231_CONTROL);
    value &= 0b11111110;
    value |= (enabled << 0); //Bit 0: Alarm 1 Interrupt Enable (A1IE)
	if(enabled)
		value |= (enabled << 2); //Bit 2: Interrupt Control (INTCN). 
	write_i2c_register(DS3231_ADDRESS, DS3231_CONTROL, value);
}